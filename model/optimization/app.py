...
from arch import arch_model  # Importing GARCH model for volatility prediction
...
# GARCH function for volatility prediction
...
def predict_portfolio():
    try:
        ...
        tickers_input = data['tickers']  # Array of tickers for filtering
        ...
        PCA_Predicted_Prices_filtered = PCA_Predicted_Prices[tickers_input]
        PCA_Actual_Prices_filtered = PCA_Actual_Prices[tickers_input]
        ...
        # Get the final weights dictionary
        final_weights_dict = {tickers_input[i]: weights_history[-1][i] for i in range(len(tickers_input))}

        # Forecast portfolio volatility using GARCH
        vol_forecast_result = forecast_portfolio_volatility(PCA_Actual_Prices_filtered.reset_index(), final_weights_dict)

        predicted_portfolio_volatility = vol_forecast_result["portfolio_volatility"]
        individual_vols = vol_forecast_result["individual_vols"]
        ...
        for period_weights in weights_history:
            ...
            for i, asset in enumerate(asset_names):
                if period_weights[i] > 0.01:
                    period_dict[asset] = round(period_weights[i] * 100, 2)
        ...
        return jsonify({
            ...
            'final_optimal_weights': final_weights_dict,
            'predicted_portfolio_volatility': round(predicted_portfolio_volatility, 4),
