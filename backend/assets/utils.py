...
def get_stock_data(ticker, api_key):
    ...
    try:
        ...
        quote_url = f"https://financialmodelingprep.com/api/v3/quote/{ticker}?apikey={api_key}"
        response = urlopen(quote_url, context=ctx)
    except TypeError:
        # Python 2 approach (no context/cafile support)
        quote_url = f"https://financialmodelingprep.com/api/v3/quote/{ticker}?apikey={api_key}"
        response = urlopen(quote_url)
    ...
    try:
        ...
        try:
            response = urlopen(ratios_url, context=ctx)
        except TypeError:
            response = urlopen(ratios_url)
    ...
    try:
        ...
        try:
            response = urlopen(metrics_url, context=ctx)
        except TypeError:
    ...
    try:
        ...
        try:
            response = urlopen(profile_url, context=ctx)
        except TypeError:
